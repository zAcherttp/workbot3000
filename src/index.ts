import { Client, GatewayIntentBits, TextChannel, GuildMember, ActivityType } from 'discord.js';
...
import { createGeminiClient, GeminiClient } from './gemini.js';
...
class WorkBot3000 {
   ...
   */
  private async onPresenceUpdate(_oldPresence: any, newPresence: any): Promise<void> {
    ...
    const isPlayingSatisfactory = this.isPlayingSatisfactoryFromPresence(newPresence);
    ...
    this.logger({
      ...
      context: { 
        ...
        activities: newPresence.activities?.map((a: GameActivity) => a.name) || []
      }
    });
  ...
  private async findTargetChannel(): Promise<void> {
    ...
    this.logger({
      ...
      context: { channelName: this.targetChannel.name, channelId: this.config.channelId }
    });
  }
   ...
   */
  private userHasChannelAccess(member: GuildMember, channel: TextChannel): boolean {
    try {
  ...
  private isPlayingSatisfactoryFromPresence(presence: any): boolean {
    ...
    return presence.activities.some((activity: GameActivity) => 
  ...
  private async performMonitoringCycle(): Promise<void> {
    try {
      ...
      if (this.config.localUserId && this.monitoredUsers.has(this.config.localUserId)) {
        await this.updateSessionState(this.config.localUserId, localUserPlaying, 'process');
      }
      
      // Check presence for all monitored users
      for (const [userId, discordUser] of this.monitoredUsers) {
        if (discordUser.member?.presence) {
          const isPlaying = this.isPlayingSatisfactoryFromPresence(discordUser.member.presence);
          await this.updateSessionState(userId, isPlaying, 'presence');
        }
      }
      
      // Log performance metrics periodically
      if (Date.now() % (60000) < this.config.pollingInterval * 1000) { // Every minute
   ...
   */
  private async updateSessionState(userId: string, isPlaying: boolean, source: 'presence' | 'process'): Promise<void> {
    ...
    if (!isPlaying && currentState.isPlaying && currentState.startTime) {
      ...
      this.logger({
        ...
        context: { userId, duration, formattedDuration: formatDuration(duration) }
      });
    }
  }
   ...
   */
  private async sendSessionEndMessage(userId: string, duration: number, _startTime: number): Promise<void> {
    if (!this.targetChannel) return;
  ...
  private async handleShutdownSessions(): Promise<void> {
    const activeSessions = Array.from(this.sessionStates.entries())
      .filter(([_, state]) => state.isPlaying && state.startTime);
  ...
  private logPerformanceMetrics(): void {
    const metrics: PerformanceMetrics = {
      ...
      monitoredUsers: this.monitoredUsers.size,
      activeSessions: Array.from(this.sessionStates.values()).filter(s => s.isPlaying).length,
      apiResponseTimes: {
